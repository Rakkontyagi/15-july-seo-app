...
export class ProhibitedPhraseDetector {
  ...
  eliminateProhibitedPhrases(content: string): string {
    ...
    sortedPhrases.forEach(item => {
      ...
      newContent = newContent.replace(regex, (match) => {
        return this.getContextAwareReplacement(match, item.replacementSuggestions);
      });
    });
  ...
  private getContextAwareReplacement(match: string, suggestions: string[], context?: string): string {
    // If no suggestions or empty suggestion, remove the phrase
    ...
    // Context-aware selection based on surrounding text
    if (context) {
      return this.selectBestReplacement(match, suggestions, context);
    }
  ...
  private selectBestReplacement(match: string, suggestions: string[], context: string): string {
    const contextWords = context.toLowerCase().split(/\s+/);
    const matchIndex = contextWords.indexOf(match.toLowerCase());
    
    // Analyze context for better replacement selection
    const contextAnalysis = {
      isTechnical: this.isTechnicalContext(contextWords),
      isFormal: this.isFormalContext(contextWords),
      isAction: this.isActionContext(contextWords, matchIndex),
      tense: this.determineTense(contextWords, matchIndex),
      subject: this.findNearestSubject(contextWords, matchIndex)
    };
    
    // Score each suggestion based on context
    const scoredSuggestions = suggestions.map(suggestion => ({
      text: suggestion,
      score: this.calculateContextScore(suggestion, contextAnalysis, contextWords)
    }));
    
    // Return the highest scoring suggestion
    scoredSuggestions.sort((a, b) => b.score - a.score);
    return scoredSuggestions[0].text;
  }

  private isTechnicalContext(contextWords: string[]): boolean {
    const technicalIndicators = [
      'algorithm', 'system', 'software', 'technology', 'technical', 'implementation',
      'framework', 'architecture', 'database', 'api', 'integration', 'optimization',
      'performance', 'security', 'scalability', 'infrastructure', 'protocol'
    ];
    
    return contextWords.some(word => technicalIndicators.includes(word));
  }

  private isFormalContext(contextWords: string[]): boolean {
    const formalIndicators = [
      'therefore', 'furthermore', 'consequently', 'moreover', 'nevertheless',
      'however', 'subsequently', 'accordingly', 'specifically', 'particularly'
    ];
    
    return contextWords.some(word => formalIndicators.includes(word));
  }

  private isActionContext(contextWords: string[], matchIndex: number): boolean {
    if (matchIndex === -1) return false;
    
    const actionVerbs = [
      'implement', 'create', 'build', 'develop', 'design', 'execute',
      'perform', 'conduct', 'manage', 'handle', 'process', 'analyze'
    ];
    
    // Check words around the match for action verbs
    const checkRange = 3;
    const start = Math.max(0, matchIndex - checkRange);
    const end = Math.min(contextWords.length, matchIndex + checkRange);
    
    for (let i = start; i < end; i++) {
      if (actionVerbs.includes(contextWords[i])) {
        return true;
      }
    }
  ...
  private determineTense(contextWords: string[], matchIndex: number): 'past' | 'present' | 'future' | 'unknown' {
    if (matchIndex === -1) return 'unknown';
    
    const pastIndicators = ['was', 'were', 'had', 'did', 'went', 'came', 'saw'];
    const presentIndicators = ['is', 'are', 'am', 'do', 'does', 'go', 'come', 'see'];
    const futureIndicators = ['will', 'shall', 'going to', 'would', 'could', 'should'];
    ...
    for (let i = start; i < end; i++) {
      ...
      if (pastIndicators.includes(word)) return 'past';
      if (presentIndicators.includes(word)) return 'present';
      if (futureIndicators.includes(word)) return 'future';
    }
    
    return 'unknown';
  }

  private findNearestSubject(contextWords: string[], matchIndex: number): string | null {
  ...
  private calculateContextScore(suggestion: string, analysis: any, contextWords: string[]): number {
    let score = 0;
    ...
    if (analysis.isTechnical) {
      const technicalWords = ['precise', 'detailed', 'comprehensive', 'systematic', 'optimized'];
      if (technicalWords.includes(suggestion.toLowerCase())) {
        score += 15;
      }
    }
    
    // Formal context bonus
    if (analysis.isFormal) {
      const formalWords = ['thorough', 'comprehensive', 'detailed', 'systematic'];
      if (formalWords.includes(suggestion.toLowerCase())) {
        score += 10;
      }
    }
    
    // Action context bonus
    if (analysis.isAction) {
      const actionWords = ['implement', 'execute', 'perform', 'conduct', 'manage'];
      if (actionWords.includes(suggestion.toLowerCase())) {
        score += 12;
      }
    }
    
    // Tense appropriateness
    if (analysis.tense === 'present' && suggestion.endsWith('ing')) {
      score += 8;
    }
    
    // Length penalty for very short suggestions in formal contexts
    if (analysis.isFormal && suggestion.length < 4) {
      score -= 5;
    }
    
    // Semantic similarity bonus (simple word matching)
    const suggestionWords = suggestion.toLowerCase().split(/\s+/);
    const contextMatches = suggestionWords.filter(word => contextWords.includes(word));
