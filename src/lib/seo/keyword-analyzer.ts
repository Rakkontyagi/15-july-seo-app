...
// Simple implementations to replace natural package
...
export function findKeywordVariations(text: string, keyword: string): string[] {
  ...
  const tokenizer = new SimpleWordTokenizer();
  const words = tokenizer.tokenize(text.toLowerCase());
  const keywordStem = SimplePorterStemmer.stem(keyword.toLowerCase());

  const variations = words?.filter(word => SimplePorterStemmer.stem(word) === keywordStem);
...
export function getKeywordDistribution(text: string, keyword: string): number[] {
  ...
  return sentences.map(sentence => {
    ...
    return words?.filter(word => SimplePorterStemmer.stem(word) === keywordStem).length || 0;
  });
}
...
export class PrecisionKeywordAnalyzer {
  ...
  private findVariations(words: string[], keyword: string): number {
    const keywordStem = SimplePorterStemmer.stem(keyword.toLowerCase());
    return words.filter(word => SimplePorterStemmer.stem(word) === keywordStem && word !== keyword.toLowerCase()).length;
  }

  private findStemmedMatches(words: string[], keyword: string): number {
    const keywordStem = SimplePorterStemmer.stem(keyword.toLowerCase());
    return words.filter(word => SimplePorterStemmer.stem(word) === keywordStem).length;
  }
