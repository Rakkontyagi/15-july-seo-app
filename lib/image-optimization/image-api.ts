...
export class ImageAPIService {
  ...
  getImageUrl(
    ...
    // Map format to CDN-supported format
    const mapFormat = (format?: string): 'auto' | 'webp' | 'avif' | 'jpeg' => {
      if (!format || format === 'auto') return 'auto';
      if (format === 'png') return 'jpeg'; // Map PNG to JPEG for CDN
      if (['webp', 'avif', 'jpeg'].includes(format)) return format as 'webp' | 'avif' | 'jpeg';
      return 'auto';
    };
