...
const queryTracker = new QueryTracker(null); // Redis instance will be set later if available
    ...
    for (let i = 0; i < keywords.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (keyword) => {
        try {
          ...
          return { keyword, error: error instanceof Error ? error.message : 'Unknown error' };
        }
      });
